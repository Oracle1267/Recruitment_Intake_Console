...
This guide gets the Recruitment Intake Console online at:
...
- The FastAPI backend is protected by `RUSHINTEL_API_KEY`.
...
- The app login uses one shared chapter password stored in `RUSHINTEL_APP_PASSWORD`.
...
4. During setup, Render will ask for `RUSHINTEL_APP_PASSWORD`.
...
| Type | `CNAME Record` |
| Host | `recruitment` |
| Value | the Render target for `rushintel-web`, for example `rushintel-web.onrender.com` |
...
1. Return to Render.
2. Open `rushintel-web` custom domains.
...
Use the shared password from `RUSHINTEL_APP_PASSWORD`.
...
1. Open Render.
2. Open the `rushintel-web` service.
3. Go to **Environment**.
4. Change `RUSHINTEL_APP_PASSWORD`.
